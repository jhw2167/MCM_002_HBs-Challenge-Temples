...
public class ChallengeLava extends LiquidBlock {
    public ChallengeLava(FlowingFluid fluid) {
        super(fluid, BlockBehaviour.Properties.copy(Blocks.LAVA)
    ...
    public void entityInside(BlockState state, Level level, BlockPos pos, Entity entity) {
        if (!entity.fireImmune()) {
            entity.hurt(level.damageSources().lava(), 6.0F); // 2x normal lava damage
        }
    }
